...
export default function SeedFundGrantsReportPage() {
  ...
  const handleDelete = async (id: string, title: string, facultyName: string) => {
    ...
    try {
      const url = `/api/admin/reports/seed-fund-grants/${id}`
  ...
  const fetchGrants = useCallback(async (year: string, dept: string, tok: string) => {
    try {
      ...
      const res = await fetch(
        `/api/admin/reports/seed-fund-grants?year=${yearQ}&dept=${deptQ}&_t=${Date.now()}`,
      ...
      if (res.ok) {
        ...
        console.error('Fetch grants failed:', res.status, errData)
        alert(`Failed to fetch grants: ${errData.error || res.statusText}`)
      }
  ...
  const exportGrants = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/reports/seed-fund-grants/export?${params}`)
      ...
      a.download = `SeedFundGrants_${label.length ? label.join('_') : 'All'}.xlsx`
      ...
      console.error(err)
      alert('Failed to export seed fund grants')
    }
  }
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/reports/seed-fund-grants/import', {
        method: 'POST',
      ...
      if (res.ok) {
        alert(`Import successful! Imported ${result.imported} records. Skipped ${result.skipped} duplicates/subtotal rows.`)
        await fetchGrants(activeYear, activeDept, token)
      } else {
      ...
      console.error(err)
      alert('Failed to import seed fund grants')
    } finally {
      setImporting(false)
    ...
    <div className="min-h-screen bg-slate-50 p-6 font-sans text-slate-900">
      <div className="w-full mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-[#0A3D8F]">Seed Fund Grants</h1>
            <p className="text-slate-500 text-sm mt-1">Historical seed-fund grants (2021 onwards) — legacy records before the live workflow</p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          {/* Total banner — matches Reports Overview card (legacy portion) */}
          <div className="bg-gradient-to-r from-[#0A3D8F]/5 to-[#0A3D8F]/10 border-b border-slate-200 px-6 py-3 flex items-center justify-between">
            <div>
              <span className="text-sm font-semibold text-slate-600">
                Total Amount Sanctioned {(search || activeDept !== 'all' || activeYear !== 'all') ? '(filtered)' : '(all records)'}
              </span>
              {!search && activeDept === 'all' && activeYear === 'all' && (
                <span className="ml-3 text-xs text-slate-400">→ This is the legacy portion of the Reports Overview &quot;Seed Fund Grants&quot; total</span>
              )}
            </div>
            <span className="text-xl font-black text-[#0A3D8F]">{formatCurrency(pageTotal)}</span>
          </div>
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-[#0A3D8F] text-white">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold border-r border-blue-800 min-w-[300px]">Title of the Project</th>
                  <th className="px-4 py-3 font-semibold border-r border-blue-800 min-w-[200px]">Faculty Name</th>
                  <th className="px-4 py-3 font-semibold border-r border-blue-800">Duration</th>
                  <th className="px-4 py-3 font-semibold border-r border-blue-800 text-right">Amount Sanctioned (₹)</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredGrants.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="px-4 py-10 text-center text-slate-500 font-medium">
                      {grants.length === 0
                        ? 'No seed fund grants found. Upload "Seed Money, Grants Received.xlsx" to import.'
                  ...
                  <tr key={g.id} className="hover:bg-blue-50 transition-colors">
                    ...
                    <td className="px-4 py-3 border-r border-slate-100 text-slate-500 text-center">{g.s_no || '—'}</td>
                    <td className="px-4 py-3 border-r border-slate-100 font-bold text-[#0A3D8F] text-center">{g.academic_year || '—'}</td>
                    <td className="px-4 py-3 border-r border-slate-100 text-slate-600">{g.dept || '—'}</td>
                    <td className="px-4 py-3 border-r border-slate-100 font-medium text-[#0A3D8F] whitespace-normal min-w-[300px]">{g.project_title || '—'}</td>
                    <td className="px-4 py-3 border-r border-slate-100 text-slate-700 whitespace-normal min-w-[200px]">{g.faculty_name || '—'}</td>
                    <td className="px-4 py-3 border-r border-slate-100 text-slate-600 text-xs">{g.duration || '—'}</td>
                    <td className="px-4 py-3 border-r border-slate-100 text-right font-bold text-green-700">
                      {g.amount_sanctioned != null ? `₹${Number(g.amount_sanctioned).toLocaleString('en-IN')}` : '—'}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <button 
                        onClick={() => handleDelete(g.id, g.project_title, g.faculty_name)}
